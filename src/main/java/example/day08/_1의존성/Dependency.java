...
class SampleControler2{ // 다른 클래스의 메소드 호출방법
...
// ============== [방법3] IOC 와 DI
@Component // + 스프링 컨테이너(저장소)에 빈(객체/인스턴스) 등록 , 스프링 실행시 자동으로 인스턴스 생성
class SampleDao3{
    void method1(){}
}
// DI 방법1
class  SampleController3{
    @Autowired // + 스프링 컨테이너(저장소)에 등록된 빈 꺼내서 멤버변수에 주입
...
class SampleController4{
    private final SampleDao3 sampleDao3; //final(수정불가능 키워드)사용함으로 불변성(안전)보장
    @Autowired // 생성자에 빈 주입 함으로써
    public SampleController4( SampleDao3 sampleDao3 ){ this.sampleDao3 = sampleDao3; }
