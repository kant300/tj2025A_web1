...
class SuperClass{ // 상위클래스 // 부모
     public void method1(){} // 메소드 == 함수 == 행위 == 이벤트
}// class e
class SubClass extends SuperClass{ // 하위 클래스 // 자식
    // extends란 ? 현재 클래스가 extends뒤에 오는 클래스로 부터 상속받는다.
    // 상속이란 ? 현실과 동일하게 상위/부모에게 멤버변수/메소드를 *물려받는 행위*
    // 오버라이딩이 이란 ? 부모로부터 물려받은 메소드를 커스텀/재정의 하는 행위
...
// [2] 어노테이션 만들기
@Retention( RetentionPolicy.RUNTIME) // 런타임(실행중)가지 유지 뜻
...
public class Example1 {
    public static void main(String[] args) {
        ...
        subClass.method2(); // <----자동완성할때 메소드명에 취소선 보인다
    }// main e
}// class e
