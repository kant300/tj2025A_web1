...
// [1] 공공데이터 , 인천시 동구 약국 (위도,경도나온다), 위도경도 안나오면 소재지를 위도,경로로 변환하는 절차가 필요하다.
const dataAPI = async() => {
    // 1. 공공데이터 준비
...
const kakaoMap = async () => {
    ...
    // 마커 클러스터러(여러개의 마커가 겹칠때 도형으로 마커수를 표현)를 생성합니다
    ...
    const data = await reponse.json();
    console.log( data );
    // (4) map 반복문을 이용하여 마커를 하나씩 생성하여 return 한 마커를 markers변수(배열)에 대입한다.
    ...
    // (5-2) 마커 클릭 이벤트 넣기 // kakao.maps.event.addListener( markers, 'click', ()=> {} )
    ...
    // (6)**여러개 마커를 가진 markers 변수를 클러스터에 등록 .addMarkers()**
