...
// [1] 회원가입
const signup = async () => {
    // 만약에 유효성검사 체크리스트에 false가 존재하면 회원가입 진행 불가능
    // JS배열내 요소찾기 함수 : 1. .indexOf() 2. . includes()
    if (signPass.includes(false) ) { // 배열내 false가 하나라도 있다면 회원가입불가
...
// <마크업> 이벤트 : inClick( 해당 마크업을 클릭했을때 ), onkeyup( 해당 마크업에 키를 떼었을때 )
...
const phonecheck = async  () => {
    ...
    // 2. 결과메시지를 출력할 DOM객체 가져오기
    const phoneCheck = document.querySelector('.phonecheck');
    if( mphone.length != 13 ){ // 만일 입력받은 연락처가 13글자가 아니면
        // 메시지 출력 
        phoneCheck.innerHTML = `-(하이프) 포함한 13글자 연락처 입력해주세요. `
        signPass[1] = false;
        return; // 함수종료
    }
    try{ // 4. fetch 실행
